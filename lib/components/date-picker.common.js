...
function toDate_toDate(argument, dirtyOptions) {
  ...
  var date = toDate_parseDate(restDateString, year)
...
function toDate_parseDate(dateString, year) {
  // Invalid ISO-formatted year
  if (year === null) {
    return null
  }
...
var locale_Locale = /*#__PURE__*/function () {
  ...
  _createClass(Locale, [{
    ...
    value: function normalizeMasks(masks) {
      ...
      return (helpers_arrayHasItems(masks) && masks || [isString_default()(masks) && masks || 'YYYY-MM-DD']).map(function (m) {
          ...
          while (1) {
            switch (_context4.prev = _context4.next) {
              case 0:
                ...
                page = pageForDate(date); // Adjust position if not explicitly set

                if (!opts.position) {
                  if (pageIsBeforePage(page, _this6.firstPage)) {
                    opts.position = 1;
